...
{
    public class RemoveTeamFromEventInteractor : RequestHandler<RemoveTeamFromEventRequest, RemoveTeamFromEventResponse>
    {
        ...
        /// <summary>
        /// Initializes a new instance of the <see cref="RemoveTeamFromEventInteractor"/> class.
        /// </summary>
        /// <param name="logger">The logging interface.</param>
        /// <param name="guildTeamRepository">The repo interface to SQL storage.</param>
        /// <param name="appSettings">The app settings.</param>
        public RemoveTeamFromEventInteractor(
            ILogger<RemoveTeamFromEventInteractor> logger,
        ...
        protected override async Task<RemoveTeamFromEventResponse> HandleRequest(RemoveTeamFromEventRequest request, RemoveTeamFromEventResponse result, CancellationToken cancellationToken)
        {
            ...
            GuildEventTeam teamToRemoveFromEvent = (await _guildTeamEventRepository.Query(t => t.EventId == request.EventId && t.TeamId == request.TeamId)).FirstOrDefault();

            if (teamToRemoveFromEvent == null)
            {
                throw new BadRequestException($"Team: {request.TeamId} to Event: {request.EventId} link cannot be found!");
            }

            await _guildTeamEventRepository.DeleteAsync(teamToRemoveFromEvent);
